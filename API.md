...
- **作用**：按可见性规则（SPEC 4.9）返回题目列表，含提交数/通过率，登录用户附带本人每题状态。未登录也可访问。
- **鉴权**：可选（未登录返回空列表）。
- **响应** `200`：
  ```json
  {
...
- **作用**：返回题目详情（不含隐藏测试点），按可见性过滤（SPEC 4.9）。
- **路径参数**：`id` 题目 ID（正整数）。
- **鉴权**：可选。未登录、未入班学生访问教师题等不可见情形均返回 404。
- **响应** `200`：
  ```json
  {
    "ok": true,
  ...
  ```
- **错误**：`404 PROBLEM_NOT_FOUND`（不存在、不可见或未登录）。
