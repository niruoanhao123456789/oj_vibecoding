...
题目按班级隔离：
- **教师/管理员**：可查看全部题目。
- **学生**：可查看「全局题目」（`created_by` 为 NULL，如 `oj_import` 导入的题）与「本人加入班级的教师发布的题目」。全局题对所有已登录用户可见，无需加入班级；教师题需先凭邀请码加入对应班级。
- **未登录**：题目列表为空。

全局题对所有已登录用户可见（无需入班）；教师题通过学生凭教师发布的邀请码加入班级后可见。
...
- 学生端显示「加入班级」入口：输入教师邀请码加入班级，加入后方可看到对应教师的题目；全局题对所有登录用户直接可见。

### 7.4 题目详情页
- 左侧：题目描述、输入/输出格式、数据范围、学生可见样例（输入/输出）。
- 右侧：代码编辑器（textarea + 简单高亮/行号）、语言固定 C++/C（支持选择 C++ 或 C）、运行按钮、提交按钮。
- 编辑器增强：光标所在行高亮（随光标/滚动同步）；`Tab` 缩进 4 个空格（多行选区整体缩进）；输入 `(`/`[`/`{`/`"`/`'` 自动补全成对符号（引号在标识符中间时不补全）、右符自动跳过、空成对符号内退格一次删除两个。
- 自测运行（`POST /api/problems/:id/run`，不算正式提交）：默认以题目明文样例作为用例，用户可添加/编辑/删除自测用例（期望输出可留空）；运行后逐例展示结果（AC/WA/TLE/MLE/RE/无期望输出）、耗时、内存与实际/期望输出，不写入 submissions 表、不计入统计。
...
- **教师管理端**：
  ...
  - 题目管理：网页表单直接新建题目（标题/描述/样例/难度/时间内存限制 + 内联隐藏测试用例编辑，含分值）、题目列表 CRUD（编辑、删除，仅限本人发布的题）。
...
- [x] 验证：教师建班→学生凭邀请码入班→学生可见该师题目；未入班学生可见全局题但不可见教师题；学生访问管理接口返回 403

### 阶段 9：日志 + 测试
- [x] 统一日志模块：记录启动、请求、判题事件、错误；按天滚动
- [x] 接口测试脚本（shell/curl 或简单脚本）：覆盖注册/登录/题目/提交/判题/管理接口
- [x] 判题单元测试：六类结果、宽松比对、资源限制边界
- [x] 端到端冒烟测试：注册→选题→提交→判题→统计 全流程自动验证
...
- 教师创建班级并生成邀请码，学生凭邀请码加入后可见该教师题目；未入班学生可见全局题但不可见教师题。
